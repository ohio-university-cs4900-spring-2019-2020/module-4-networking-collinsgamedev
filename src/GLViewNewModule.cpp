...
void GLViewNewModule::onCreate()
{
   ...
   */
   ...
   client = NetMessengerClient::New("127.0.0.1", "101010");

   this->drone = Drone::New();
   worldLst->push_back(this->drone->get_drone());
}
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   {
	   this->drone->set_position(this->drone->get_position() + Vector(5, 5, 5));

	   if (client->isTCPSocketOpen())
	   {
		   this->client->sendNetMsgSynchronousTCP(NetMsgAntGsSetback(this->drone->get_position()));
	   }
   }
}
